    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="glass-panel p-6 rounded-xl">
           <div className="text-gray-400 text-sm font-medium mb-1">Clinical Uptime</div>
           <div className="text-3xl font-bold text-emerald-400 flex items-center gap-2">
             99.98% <CheckCircle className="w-6 h-6" />
           </div>
        </div>
        <div className="glass-panel p-6 rounded-xl">
           <div className="text-gray-400 text-sm font-medium mb-1">Clinical IoT Agents</div>
           <div className="text-3xl font-bold flex items-center gap-2 text-emerald-400">
             {hosts.length} <Heart className="w-6 h-6 text-emerald-500 animate-pulse" />
           </div>
        </div>
        <div className="glass-panel p-6 rounded-xl">
           <div className="text-gray-400 text-sm font-medium mb-1">Active Incidents</div>
           <div className={`text-3xl font-bold flex items-center gap-2 ${activeAlerts > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
             {activeAlerts} <AlertTriangle className="w-6 h-6" />
           </div>
        </div>
        <div className="glass-panel p-6 rounded-xl">
           <div className="text-gray-400 text-sm font-medium mb-1">ICU Patients Status</div>
           <div className={`text-3xl font-bold flex items-center gap-2 ${criticalPatients > 0 ? 'text-red-400 animate-pulse' : 'text-purple-400'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="flex justify-between items-center">
            ...
            <Link to="/app/hms" className="text-emerald-400 hover:text-emerald-300 text-xs font-semibold flex items-center gap-1">
              Go to HMS Portal <Zap className="w-3.5 h-3.5" />
            </Link>
          </div>
          <div className="grid grid-cols-1 gap-4">
            {hosts.map(h => (
              <Link to={`/app/host/${h.id}`} key={h.id} className="glass-panel p-5 rounded-xl hover:bg-white/5 transition-colors flex items-center justify-between cursor-pointer border border-white/5 hover:border-emerald-500/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-lg text-gray-200">{h.hostname}</h3>
                    <div className="text-xs text-gray-500">{h.type} • HL7 Listener Enabled</div>
                  </div>
                </div>
