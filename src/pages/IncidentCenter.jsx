    ...
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-muted">Track alerts and run AI Root Cause Analysis (RCA)</p>
        </div>
      </div>

      <div className="flex gap-6 flex-1 min-h-0">
        <div className="w-1/3 glass-panel rounded-xl flex flex-col overflow-hidden">
            <div className="p-4 border-b border-border font-bold bg-card/40">Incident Timeline</div>
            <div className="flex-1 overflow-auto p-2 space-y-2">
              {incidents.map(inc => (
                <div key={inc.id} className={`p-4 rounded-lg border cursor-pointer transition-colors ${inc.status === 'active' ? 'bg-red-500/10 border-red-500/20 hover:bg-red-500/20' : 'bg-card border-border hover:bg-foreground/5'}`}>
                  <div className="flex justify-between items-start mb-2">
                     <span className="font-bold text-sm">{inc.id}</span>
                     <span className="text-xs text-muted">{inc.time}</span>
                  </div>
                  <div className="text-sm font-medium mb-1">{inc.host}</div>
                  <div className="text-xs text-red-600 dark:text-red-400 bg-red-500/10 inline-block px-2 py-0.5 rounded">{inc.rule}</div>
                  ...
                  <div className="mt-4 flex gap-2">
                     {inc.status === 'active' ? (
                       <button 
                         ...
                         disabled={analyzing}
                         className="flex-1 text-xs bg-gradient-to-r from-purple-600 to-primary hover:opacity-90 py-1.5 rounded font-bold flex items-center justify-center gap-1 text-white shadow-[0_0_15px_rgba(168,85,247,0.3)]"
                       >
           ...
           {activeRca ? (
             <div className="space-y-4">
                <div className="flex items-center gap-2 text-xl font-bold border-b border-border pb-4">
                   <Sparkles className="w-6 h-6 text-purple-600 dark:text-purple-400" /> AI Diagnostic Report
                </div>
                <div className="prose max-w-none">
                  {activeRca.split('\n').map((line, i) => {
                    if (line.startsWith('###')) return <h3 key={i} className="text-lg font-bold text-foreground mt-4">{line.replace('### ', '')}</h3>;
                    if (line.startsWith('**')) return <p key={i} className="font-bold text-foreground mt-2">{line.replace(/\*\*/g, '')}</p>;
                    if (line.startsWith('1.') || line.startsWith('2.') || line.startsWith('3.')) return <div key={i} className="ml-4 my-1 text-gray-100 font-mono text-sm bg-slate-900 p-3 rounded border border-slate-800">{line}</div>;
                ...
                <div className="pt-6">
                   <button 
                     ...
                     className="bg-green-600 hover:bg-green-500 text-white font-bold px-4 py-2 rounded flex items-center gap-2 transition-colors"
                   >
